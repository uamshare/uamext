<?php namespace uamext\widgets\grid\datatable;
/**
...
class DataTableAction extends Action
{
    ...
    public $colExclude = ['rownumber','active','primarykey'];
    ...
    public function run()
    {
        ...
        $filterQuery = $this->applyFilter($filterQuery, $columns, $search, $this);
        $filterQuery = $this->applyOrder($filterQuery, $columns, $order, $this);
        ...
        try {
			...
			foreach($dataProvider->getModels() as $key => $value){
				...
				foreach($columns as $column){
					if(in_array($column['data'],$this->colExclude)) continue;
     ...
     * @return ActiveQuery
     */
    public function applyOrder(ActiveQuery $query, $columns, $order, $classModel)
    {
        if ($this->applyOrder !== null) {
            return call_user_func($this->applyOrder, $query, $columns, $order, $classModel);
        }
     ...
     */
    public function applyFilter(ActiveQuery $query, $columns, $search, $classModel)
    {
        if ($this->applyFilter !== null) {
            return call_user_func($this->applyFilter, $query, $columns, $search, $classModel);
        }
